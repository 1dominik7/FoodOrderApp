      ...
      <table className='w-full border-separate border-spacing-3'>
       ...
       {  data.map((item:OrderType) => ( 
          <tr className={`${item.status !== "delivered" && "bg-red-50"}`} key={item.id}>
            ...
            <td className='py-6 px-1'>{item.price >= 50 ? item.price : +item.price+5}</td>
            <td className='hidden md:block py-6 px-1'>{item.products[0].title}</td>
            {session?.user.isAdmin ? 
            (<td>
            <form className="flex items-center justify-center gap-4" onSubmit={(e) => handlerUpdate(e, item.id)}>
